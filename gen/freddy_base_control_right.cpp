...
int main(int argc, char **argv)
{
  ...
  double plat_vel_setpoint[3] = {0.05, 0.05, 0.25};
  ...
  while (true)
  {
    ...
    printf("dist align errors: ");
    print_array(dist_align_errors, 4);

    // ramp the ff torques based on the pivot velocities
    double tau_align_c[8]{};
    for (size_t i = 0; i < 4; i++)
    {
      tau_align_c[2 * i] = tau_align[i];
      tau_align_c[2 * i + 1] = tau_align[i];
    }

    printf("alignmnt torques: ");
    print_array(tau_align_c, 8);

    for (size_t i = 0; i < 4; i++)
    {
      if (abs(robot.mobile_base->state->pivot_velocities[i]) < pivot_vel_zero_threshold &&
          abs(tau_align[i]) > 0.0 && count > 2)
      {
        printf("adding ff tau to pivot %d\n", i);
    ...
    printf("align taus w ff:  ");
    print_array(tau_align_c, 8);

    double tau_wheel_c[8]{};
    base_fd_solver_cgls(&robot, plat_force, tau_align_c, tau_wheel_c);
